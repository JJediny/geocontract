...
# ── Worked-example round-trip (plan §5.7) ────────────────────────────────
# The full-coverage signed example at
# ``examples/groton-rhine-002.example.data.json`` is generated by
# ``scripts/build_signed_example.py`` using a documented test keypair.
# Verify that the example round-trips through the canonicalisation +
# proof verification path so the example stays honest.

import json as _json
from pathlib import Path as _P

EXAMPLES_DIR = _P(__file__).resolve().parent.parent.parent / "examples"


def test_worked_signed_example_validates() -> None:
    """The full-coverage signed example must verify with the documented test key."""
    from geocontract_tools.validate_proof import verify_proof as _vp
    ...
    # Verify must succeed.
    _vp(proposal, did_resolver=_resolver)